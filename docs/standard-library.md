...
The native modules (`image`, `noise`, `rectpack`, `font`, `audio`, `gl`,
`sdl2`, `raylib`, `sqlite`, `nn`, `gif`, `canvas`), the `zen_game` / `zen_gl`
runners that carry them, and the batteries-included `zen` CLI live in the
separate repository **zenvm-modules** (https://github.com/akadjoker/zenvm-modules),
which consumes this repo as a git submodule. This repository builds only the
core VM and the plain `zen` CLI.
