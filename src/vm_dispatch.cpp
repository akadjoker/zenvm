    ...
    switch (ZEN_OP(*ip)) \
    {
        ...
        ** ================================================================= */
        ...
        CASE(OP_NEG)
        {
            ...
            Value v = R[ZEN_B(i)];
            if (v.type == VAL_INT)
                R[ZEN_A(i)] = val_int(-v.as.integer);
            else
        ...
        CASE(OP_ADDI)
        {
            ...
            if (vb.type == VAL_INT)
                R[ZEN_A(i)] = val_int(vb.as.integer + imm);
            else
        ...
        CASE(OP_TOSTRING)
        {
            ...
            else
            {
                ...
                {
                    len = snprintf(buf, sizeof(buf), "%d", v.as.integer);
                }
                else if (is_float(v))
                {
        ...
        CASE(OP_PRINT)
        {
            uint32_t i = *ip;
            Value v = R[ZEN_A(i)];
            if (!is_nil(v)) print_value(v);
            if (ZEN_B(i)) printf("\n");   /* B=1: newline (last arg) */
            else printf(" ");             /* B=0: space separator */
            NEXT();
        }
