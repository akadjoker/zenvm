...
if (mlen == 4 && memcmp(mname, "push", 4) == 0)
{
    ...
    if (arg_count < 1) { runtime_error("push() expects at least 1 argument"); return; }
...
else if (mlen == 3 && memcmp(mname, "pop", 3) == 0)
{
    ...
    if (arr_count(arr) == 0) { runtime_error("pop() on empty array"); return; }
    R[base] = *--arr->end;
}
...
else if (mlen == 6 && memcmp(mname, "remove", 6) == 0)
{
    ...
    if (arg_count != 1 || !is_int(args[0])) { runtime_error("remove() expects 1 integer argument"); return; }
    int32_t idx = args[0].as.integer;
    int32_t count = arr_count(arr);
    if (idx < 0 || idx >= count) { runtime_error("remove() index out of bounds"); return; }
...
else if (mlen == 6 && memcmp(mname, "insert", 6) == 0)
{
    ...
    if (arg_count != 2 || !is_int(args[0])) { runtime_error("insert() expects (int, value)"); return; }
    int32_t idx = args[0].as.integer;
    int32_t count = arr_count(arr);
    if (idx < 0 || idx > count) { runtime_error("insert() index out of bounds"); return; }
...
else if (mlen == 5 && memcmp(mname, "slice", 5) == 0)
{
    ...
    if (arg_count >= 2 && is_int(args[1])) end_idx = args[1].as.integer;
...
else if (mlen == 8 && memcmp(mname, "contains", 8) == 0)
{
    ...
    if (arg_count != 1) { runtime_error("contains() expects 1 argument"); return; }
...
else if (mlen == 4 && memcmp(mname, "join", 4) == 0)
{
    ...
    for (int32_t ji = 0; ji < count; ji++) {
        if (ji > 0) total_len += sep_len;
        Value v = arr->data[ji];
        if (is_string(v)) total_len += as_string(v)->length;
        else if (is_int(v)) total_len += int_to_cstr(v.as.integer, num_buf);
        else if (is_float(v)) total_len += snprintf(num_buf, sizeof(num_buf), "%g", v.as.number);
        else if (is_nil(v)) total_len += 3;
        else if (is_bool(v)) total_len += v.as.boolean ? 4 : 5;
        else total_len += 3;
    }
    ...
    char *p = buf;
    for (int32_t ji = 0; ji < count; ji++) {
        if (ji > 0) { memcpy(p, sep, sep_len); p += sep_len; }
        Value v = arr->data[ji];
        if (is_string(v)) { memcpy(p, as_cstring(v), as_string(v)->length); p += as_string(v)->length; }
        else if (is_int(v)) { int n = int_to_cstr(v.as.integer, num_buf); memcpy(p, num_buf, n); p += n; }
        else if (is_float(v)) { int n = snprintf(num_buf, sizeof(num_buf), "%g", v.as.number); memcpy(p, num_buf, n); p += n; }
        else if (is_nil(v)) { memcpy(p, "nil", 3); p += 3; }
        else if (is_bool(v)) { const char *s = v.as.boolean ? "true" : "false"; int n = v.as.boolean ? 4 : 5; memcpy(p, s, n); p += n; }
        else { memcpy(p, "???", 3); p += 3; }
    }
...
else if (mlen == 4 && memcmp(mname, "sort", 4) == 0)
{
    ...
    if (arg_count > 1) { runtime_error("sort() expects 0 or 1 argument"); return; }
    bool descending = false;
    if (arg_count == 1) {
        if (!is_string(args[0])) { runtime_error("sort() argument must be a string (\"asc\" or \"desc\")"); return; }
...
else if (mlen == 8 && memcmp(mname, "index_of", 8) == 0)
{
    ...
    if (arg_count != 1) { runtime_error("index_of() expects 1 argument"); return; }
