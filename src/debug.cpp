...
static const char* s_opnames[] = {
    "LOADNIL",    "LOADBOOL",   "LOADK",      "LOADI",      "MOVE",
    "GETGLOBAL",  "SETGLOBAL",
    "ADD",        "SUB",        "MUL",        "DIV",        "MOD",
    "NEG",        "ADDI",       "SUBI",
    ...
    "EQ",         "LT",         "LE",         "NOT",
    "JMP",        "JMPIF",      "JMPIFNOT",
    "CALL",       "CALLGLOBAL", "RETURN",
    "CLOSURE",    "GETUPVAL",   "SETUPVAL",   "CLOSE",
    "NEWFIBER",   "RESUME",     "YIELD",
    "NEWARRAY",   "NEWMAP",
    "GETFIELD",   "SETFIELD",   "GETINDEX",   "SETINDEX",
    "NEWCLASS",   "NEWINSTANCE","GETMETHOD",
    "CONCAT",     "LEN",        "PRINT",
    "LTJMPIFNOT", "LEJMPIFNOT",
    "HALT",
};
...
const char* opcode_name(OpCode op) {
    ...
    if (idx >= 0 && idx < count) return s_opnames[idx];
    return "???";
}
